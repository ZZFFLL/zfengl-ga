...
class MemPalaceExperienceExtractorTests(unittest.TestCase):
    def test_extracts_success_steps_solution_and_verification(self):
        ...
        3. 增加低分跳过日志。
        验证：python -m pytest tests/test_webui_server.py -k "not MemPalace" -q -> passed
        ...
        self.assertIn(("verification", 'python -m pytest tests/test_webui_server.py -k "not MemPalace" -q -> passed'), pairs)
