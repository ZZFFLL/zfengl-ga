...
class WebUILogParserTests(unittest.TestCase):
    ...
    def test_parse_execution_log_ignores_turn_marker_inside_long_fenced_tool_result(self):
        text = (
            "**LLM Running (Turn 1) ...**\n"
            "<summary>\n运行脚本\n</summary>\n"
            "🛠️ Tool: `code_run`  📥 args:\n"
    ...
    def test_parse_execution_log_ignores_turn_marker_inside_nested_short_fence(self):
        text = (
            "**LLM Running (Turn 1) ...**\n"
            "<summary>\n运行脚本\n</summary>\n"
            "🛠️ Tool: `code_run`  📥 args:\n"
            ...
            "[Stdout]\n"
            "```python\n"
            "LLM Running (Turn 999) ... should stay inside nested code fence\n"
            "```\n"
            "`````\n"
            "最终回答。"
        )
    ...
    def test_project_ga_response_marks_only_latest_turn_active(self):
        ...
        self.assertEqual(projection["turns"][0]["summary"], "读取文件")
        self.assertEqual(projection["turns"][1]["summary"], "运行测试")
    ...
    def test_project_ga_response_uses_latest_ask_user_interaction(self):
        projection = project_ga_response(
            "**LLM Running (Turn 1) ...**\n"
            "🛠️ Tool: `ask_user`  📥 args:\n"
            "````text\n"
            '{"question": "旧问题", "candidates": ["旧选项"]}\n'
            "````\n"
            "`````\nWaiting for your answer ...\n`````\n"
            "**LLM Running (Turn 2) ...**\n"
            "🛠️ Tool: `ask_user`  📥 args:\n"
            "````text\n"
            '{"question": "新问题", "candidates": ["新选项"]}\n'
            "````\n"
            "`````\nWaiting for your answer ...\n`````\n",
            running=True,
        )

        self.assertEqual(projection["interaction"]["question"], "新问题")
        self.assertIn("新问题", projection["content"])
        self.assertIn("1. 新选项", projection["content"])
        self.assertNotIn("旧问题", projection["content"])
...
class WebUITaskManagerTests(unittest.TestCase):
    ...
    def test_start_chat_creates_conversation_and_streams_to_done(self):
        ...
        self.assertEqual(events[1]["execution_log"][0]["state"], "active")
        self.assertEqual(events[-2]["event"], "execution_update")
        self.assertEqual(events[-2]["execution_log"][0]["state"], "completed")
        ...
        self.assertEqual(detail["execution_log"][0]["state"], "completed")

    def test_streaming_execution_update_marks_latest_turn_active_and_persists_completed(self):
        conversation = self.store.create_conversation(initial_user_text="turn states")
        ...
        execution_event = next(event for event in events if event["event"] == "execution_update")
        self.assertEqual([turn["state"] for turn in execution_event["execution_log"]], ["completed", "active"])
        self.assertEqual(events[-2]["event"], "execution_update")
        self.assertEqual([turn["state"] for turn in events[-2]["execution_log"]], ["completed", "completed"])
        self.assertEqual(events[-1]["event"], "message_done")
        detail = self.store.get_conversation_detail(conversation["id"])
        self.assertEqual([turn["state"] for turn in detail["execution_log"]], ["completed", "completed"])
    ...
    def test_streaming_ask_user_update_emits_visible_question_in_message_body(self):
        ...
        self.assertEqual(events[-2]["event"], "execution_update")
        self.assertEqual(events[-1]["event"], "message_done")
        self.assertIn("2. 未安装", events[-1]["content"])
        self.assertIn("3. 需要协助", events[-1]["content"])
