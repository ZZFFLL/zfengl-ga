...
_PLACEHOLDER_PREFIX = "\x00WEBUI_FENCE_"
...
def _mask_fenced_blocks(text):
    """临时屏蔽 fenced block，避免工具输出里的 Turn 标记被误切。"""
    source = text or ""
    blocks = []
    result = []
    cursor = 0
    ...
    while cursor < len(source):
        ...
        fence = match.group(1)
        close_match = re.search(rf"(?m)^{re.escape(fence)}\s*$", source[body_start:])
        if not close_match:
            result.append(source[cursor:])
            break
        end = body_start + close_match.end()
        result.append(source[cursor:start])
        blocks.append(source[start:end])
        result.append(f"{_PLACEHOLDER_PREFIX}{len(blocks) - 1}\x00")
        cursor = end

    return "".join(result), blocks


def _unmask_fenced_blocks(text, blocks):
    return re.sub(
        rf"{re.escape(_PLACEHOLDER_PREFIX)}(\d+)\x00",
        lambda match: blocks[int(match.group(1))],
        text or "",
    )
...
def _parse_ask_user_payload(tool_call):
    ...
    if not question and not candidate_lines:
        return None
    return {
        "type": "ask_user",
        "question": question,
...
def _render_ask_user_visible_text(text):
    ...
    for tool_call in reversed(_parse_tool_calls(text)):
...
def _render_ask_user_tool_call(tool_call):
    payload = _parse_ask_user_payload(tool_call)
    if not payload:
        return ""
    question = payload["question"]
    candidate_lines = payload["candidates"]
    lines = []
    if question:
...
def _decorate_tool_call(tool_call):
    decorated = dict(tool_call or {})
    result = decorated.get("result") or ""
    decorated["result_length"] = len(result)
    decorated["result_preview"] = smart_truncate_result(result)
    return decorated


def smart_truncate_result(text, limit=1200):
    text = (text or "").strip()
    if len(text) <= limit:
        return text
...
def _decorate_turns(turns, running=False):
    decorated = []
    active_index = len(turns) - 1 if running and turns else -1
    for index, turn in enumerate(turns):
        next_turn = dict(turn)
        next_turn["summary"] = next_turn.get("content", "")
        next_turn["state"] = "active" if index == active_index else "completed"
        next_turn["tool_calls"] = [_decorate_tool_call(item) for item in next_turn.get("tool_calls", [])]
        decorated.append(next_turn)
    return decorated


def _extract_interaction(turns):
    for turn in reversed(turns or []):
        for tool_call in reversed(turn.get("tool_calls") or []):
            payload = _parse_ask_user_payload(tool_call)
            if payload:
                return payload
    return None
...
def render_message_content(content, execution_log=None, role=None):
    ...
    if cleaned:
        return cleaned
    if role == "assistant":
        for turn in reversed(execution_log or []):
            for tool_call in reversed(turn.get("tool_calls") or []):
                visible = _render_ask_user_tool_call(tool_call)
                if visible:
                    return visible
...
def parse_execution_log(text):
    ...
    masked, fenced_blocks = _mask_fenced_blocks(text or "")
    parts = list(_TURN_RE.finditer(masked))
    turns = []
    for idx, match in enumerate(parts):
        turn = int(match.group(1))
        start = match.end()
        end = parts[idx + 1].start() if idx + 1 < len(parts) else len(masked)
        content = _unmask_fenced_blocks(masked[start:end], fenced_blocks).strip()
...
def project_ga_response(raw_text, running=False):
    """将 GA 原始文本快照投影为 WebUI 可渲染的消息模型。"""
    turns = _decorate_turns(parse_execution_log(raw_text), running=running)
    return {
        "content": extract_visible_reply_text(raw_text),
        "turns": turns,
        "interaction": _extract_interaction(turns),
    }
    ...
    def drain_task(self, task_id, timeout=10.0):
        ...
        while task.status in {"running", "aborted"}:
            ...
            if "next" in item:
                ...
                task.execution_log = projection["turns"]
                cleaned = projection["content"]
            ...
            if "done" in item:
                ...
                task.completed_at = time.time()
                self.last_reply_time = int(task.completed_at)
                projection = project_ga_response(task.current_response, running=False)
                task.execution_log = projection["turns"]
                cleaned = projection["content"]
...
def main(argv=None):
    ...
    parser.add_argument("--port", type=int, default=18610)
